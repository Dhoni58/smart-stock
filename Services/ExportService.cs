...
public class ExportService
{
    ...
    public byte[] ExportProductsToExcel(List<Product> products)
    {
        ...
        sheet.Cell(1, 3).Value = "Cena";
        sheet.Cell(1, 4).Value = "Na skladě";
        sheet.Cell(1, 5).Value = "Min. zásoba";
        sheet.Cell(1, 6).Value = "Stav";
        ...
        for (int i = 0; i < products.Count; i++)
        {
            ...
            sheet.Cell(row, 4).Value = p.WarehouseInv;
            sheet.Cell(row, 5).Value = p.MinimumInv;
            sheet.Cell(row, 6).Value = p.WarehouseInv <= p.MinimumInv ? "Nízká zásoba" : "OK";
        ...
        var document = Document.Create(container =>
        {
            container.Page(page =>
            {
                ...
                page.Content().Table(table =>
                {
                    ...
                    table.Header(header =>
                    {
                        foreach (var col in new[] { "Název", "Kategorie", "Cena", "Na skladě", "Minimum", "Stav" })
                        {
                    ...
                    foreach (var p in products)
                    {
                        ...
                        table.Cell().Background(bg).Padding(5).Text(p.Name);
                        table.Cell().Background(bg).Padding(5).Text(p.Category?.Name ?? "—");
                        table.Cell().Background(bg).Padding(5).Text($"{p.Price:N2} Kč");
                        table.Cell().Background(bg).Padding(5).Text(p.WarehouseInv.ToString());
                        table.Cell().Background(bg).Padding(5).Text(p.MinimumInv.ToString());
